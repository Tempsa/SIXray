...
function M.parse(arg)
    ...
    cmd:text('Options:')
    ------------ General options --------------------
    cmd:option('-data',         '/root/Share/G2/X-ray-dataset',            'Path to dataset')
    cmd:option('-dataset',      'X-ray-cls', 'Options: voc2007-cls')
    ...
    cmd:option('-gpuDevice',     '{1,2,3,4}',      'index of using gpuDevice ')
    ------------- Training options --------------------
    cmd:option('-nEpochs',         0,       'Number of total epochs to run')
    cmd:option('-epochStep',         50,      'every epochStrp LR*LRratio')
    ...
    cmd:option('-LRRatio',           0.5,   'learning rate decay ratio')
    cmd:option('-LRp',             1,     'learning rate for pretrained layers')
    cmd:option('-momentum',        0.9,   'momentum')
    cmd:option('-weightDecay',     1e-4,  'weight decay')
    ---------- Model options ----------------------------------
    cmd:option('-netType',      'SPResNet18',         'Options: resnet | preresnet')
    cmd:option('-netInit',      '',               'Path to inital model')
    cmd:option('-depth',        34,               'ResNet depth: 18 | 34 | 50 | 101 | ...', 'number')
    cmd:option('-shortcutType', '',               'Options: A | B | C')
    cmd:option('-retrain',      'none',           'Path to model to retrain with')
    cmd:option('-optimState',   'none',           'Path to an optimState to reload from')
    cmd:option('-loss',         'MultiLabel',   'loss')
    ---------- Model options ----------------------------------
    cmd:option('-shareGradInput',  'false',       'Share gradInput tensors to reduce memory usage')
    cmd:option('-optnet',          'false',       'Use optnet to reduce memory usage')
    cmd:option('-resetClassifier', 'false',       'Reset the fully connected layer for fine-tuning')
    cmd:option('-nClasses',        5,             'Number of classes in the dataset')
    ---------- My options ----------------------------------
    cmd:option('-preprocessing',    'warp',         'image preprocessing')
    cmd:option('-results',          './results/',             'path to write results')
	cmd:option('-localization',     'true',        'path to write results')
    cmd:option('-train',            'multilabel',   'train with [multiclass | multilabel]')
