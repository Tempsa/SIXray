...
function Trainer:test(epoch, dataloader)
	...
	local outmap
	 --cal imputmap = torch.zeros(nImages,3,224,224)
	------===============================================---------
	if opt.testOnly then
		outmap=torch.zeros(nImages, self.opt.nClasses,14,14):float()
	end
	------===============================================---------
	...
	for n, sample in dataloader:run() do
		...
		--print(name)
		if opt.testOnly then
	        local activations = self.model.modules[19].output
	        local weights =self.model.modules[23].weight
	        for j=1,self.opt.nClasses do
	        	local weight = weights[{j,{}}]
				weight = weight:view(1,(#weights)[2],1,1):expandAs(activations):clone()
				local cam = activations:clone():cmul(weight)
				cam = cam:sum(2)

				outmap[{{N+1,N+batchSize},{j},{},{}}]:add(cam:float())
	        end
    	end
        --------------------------------------------------------------------------------
       --mputmap[{{N+1,N+batchSize},{},{},{}}]:add(input:float())
		scores[{{N+1,N+batchSize},{}}] = output[1]:float()
	...
	if  opt.testOnly then
	    matio.save('SP-cam-imsize224-xray-testmap-v2.mat',outmap)
	 	matio.save('SP-cam-imsize224-xray-scores-v2.mat',scores)
	 	--tio.save('imput.mat',imputmap)
	 	--rch.save('name.t7',name)
	 	--rch.save('imput.t7',imputmap)
...
function Trainer:writeScores(epoch, dataloader, split)
    ...
    --sys.execute('mkdir -p'.. pathResults)
...
function Trainer:learningRate(epoch)
	-- Training schedule
	if epoch % self.opt.epochStep ==0 then
		self.opt.LR = self.opt.LR*self.opt.LRRatio
	end
