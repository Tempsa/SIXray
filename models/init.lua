...
function M.setup(opt, checkpoint)
    ...
    if opt.nGPU > 1 then
        ...
        gpus=opt.gpuDevice
        --local gpus = torch.range(2, 4):totable()
        ...
        :threads(function()
            local cudnn = require 'cudnn'
                require 'cuspn'
                require 'spn'
                require 'spatial-pooling'   --add defined layer
    ...
    local criterion = {}
    if opt.loss == 'CrossEntropy' then
        criterion = nn.CrossEntropyCriterion()
    elseif opt.loss == 'MultiLabelSoftMarginCriterion' or opt.loss == 'MultiLabel' then
        --criter1 = nn.MultiLabelSoftMarginCriterion()
        --criter2 = nn.MultiLabelSoftMarginCriterion()
        --criter3 = nn.MultiLabelSoftMarginCriterion()
        criterion = nn.ParallelCriterion():add(nn.MultiLabelSoftMarginweightCriterion()):add(nn.MultiLabelSoftMarginweightCriterion()):add(nn.MultiLabelSoftMarginweightCriterion())
    ...
    if opt.backend == 'nn' then
        ...
        if opt.featureNet ~= nil then
            opt.featureNet = opt.featureNet:float()
        end
    else
        if opt.backend == 'cudnn' then
            cudnn.convert(model, cudnn)
            if opt.featureNet ~= nil then
                cudnn.convert(opt.featureNet, cudnn)
            end
        end
        model = model:cuda()
        criterion = criterion:cuda()
        if opt.featureNet ~= nil then
            opt.featureNet = opt.featureNet:cuda()
        end
    end
